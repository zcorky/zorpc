...
const app = new Koa();
// all koa middleware should return Promise, and await next, not like koa
...
const fetch = (url: string, options: RequestInit) => {
  ...
  return new Promise<{ json: () => Promise<any>}>((resolve, reject) => {
    ...
    request(app.callback())
      ...
      .expect(200)
      .end((error, res) => {
        // console.log(`supertest done: `, path, options, error);
...
describe("@zorpc/channel-express", () => {
  ...
  rpc.server.prepare().then((config) => {
    ...
    // });
    app.use(async (ctx, next) => {
      // console.log('request 2: ', ctx.path, ctx.request.body);
      await next();
    });
    app.use(body({ json: true }));
    // app.use(bodyParser());
    app.use(async (ctx, next) => {
      // console.log('request 3: ', ctx.path, ctx.request.body);
      await next();
    });
